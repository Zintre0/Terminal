...
CREATE TABLE IF NOT EXISTS `TERMINAL_DE_BUSES`.`EMPRESA` (
  ...
  `COBRO_SERVICIO_idCOBRO_SERVICIO` INT NULL,
  `NOMBRE_EMPRESA` VARCHAR(45) NOT NULL,
  `RUT_EMPRESA` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idEMPRESA`),
  INDEX `fk_EMPRESA_PAGO_idx` (`PAGO_idPAGO` ASC),
  INDEX `fk_EMPRESA_COBRO_SERVICIO1_idx` (`COBRO_SERVICIO_idCOBRO_SERVICIO` ASC),
  INDEX `fk_EMPRESA_GIRO1_idx` (`GIRO_idGIRO` ASC),
  CONSTRAINT `fk_EMPRESA_PAGO`
    FOREIGN KEY (`PAGO_idPAGO`)
    REFERENCES `TERMINAL_DE_BUSES`.`PAGO` (`idPAGO`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE,
  CONSTRAINT `fk_EMPRESA_COBRO_SERVICIO1`
    FOREIGN KEY (`COBRO_SERVICIO_idCOBRO_SERVICIO`)
    REFERENCES `TERMINAL_DE_BUSES`.`COBRO_SERVICIO` (`idCOBRO_SERVICIO`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE,
